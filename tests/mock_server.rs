...
use tiled_rs::client::{Context, ContextOptions, HttpCache};
...
async fn client_resolver_emits_custom_variant() {
    use std::any::Any;
    use tiled_rs::client::any_client::{AnyClient, ClientResolver};
    use tiled_rs::client::base::Item;
    ...
    impl ClientResolver for Resolver {
        ...
        ) -> Option<tiled_rs::client::Result<std::sync::Arc<dyn Any + Send + Sync>>> {
            if item
                .attributes
                .specs
                .as_deref()
    ...
    let url = url::Url::parse(&format!("{base}/api/v1/metadata/")).unwrap();
    let resp = ctx.get(&url).await.unwrap();
    let envelope: serde_json::Value = tiled_rs::client::utils::decode_response(resp)
        .await
        .unwrap();
...
fn any_client_custom_clone_is_safe() {
    ...
    use tiled_rs::client::any_client::AnyClient;
...
async fn get_within_active_search_routes_through_keylookup() {
    use tiled_rs::client::queries::Key;
    use tiled_rs::client::{AnyClient, ContainerClient, Context, error::ClientError};
...
async fn awkward_read_buffers_posts_sorted_form_keys() {
    use tiled_rs::client::{AwkwardClient, Context, Item};
