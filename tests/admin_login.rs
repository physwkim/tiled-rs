...
async fn build_test_router() -> (axum::Router, tempfile::TempDir, tiled_rs::auth::Issuer) {
    build_test_router_with(false).await
}

async fn build_test_router_with(
    trust_forwarded_proto: bool,
) -> (axum::Router, tempfile::TempDir, tiled_rs::auth::Issuer) {
    let dir = tempfile::tempdir().unwrap();
    let auth_uri = format!("sqlite://{}", dir.path().join("auth.db").display());
    let auth_db = tiled_rs::auth::AuthDb::connect(&auth_uri).await.unwrap();
    auth_db.migrate().await.unwrap();

    let mut dummy = tiled_rs::auth::DummyAuthenticator::new("dummy");
    dummy.add_user("alice", "s3cret").unwrap();

    let issuer = tiled_rs::auth::Issuer::new(b"test-secret-that-is-32bytes-long!!").unwrap();

    let state = tiled_rs::web::WebState {
    ...
    (tiled_rs::web::build_router(state), dir, issuer)
}
...
async fn login_caps_session_scopes_to_role() {
    ...
    assert!(
        !claims.scopes.contains(tiled_rs::auth::Scope::Admin),
        "non-admin login must not be granted Admin scope, got {:?}",
        claims.scopes,
    );
    assert!(
        claims.scopes.contains(tiled_rs::auth::Scope::ReadData),
