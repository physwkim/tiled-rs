...
use tiled_catalog::adapter::UnresolvedLeaf;
use tiled_catalog::data_source::{AssetSpec, DataSourceSpec};
use tiled_catalog::{Catalog, CatalogAdapter, RegisterRequest};
...
async fn catalog_app(expose_raw_assets: bool) -> (axum::Router, Catalog, tempfile::TempDir) {
    ...
    (tiled_server::build_app(state), catalog, dir)
}
...
fn no_catalog_app() -> axum::Router {
    ...
    tiled_server::build_app(make_state(root, None, true))
}
