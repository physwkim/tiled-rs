...
use tiled_catalog::webhook::WebhookCreate;
...
impl From<tiled_catalog::orm::Webhook> for WebhookResponse {
    fn from(w: tiled_catalog::orm::Webhook) -> Self {
        Self {
            id: w.id,
            node_id: w.node_id,
            url: w.url,
...
pub async fn register(
    ...
    auth.require(tiled_auth::Scope::WriteWebhooks)?;
...
pub async fn list(
    ...
    auth.require(tiled_auth::Scope::ReadWebhooks)?;
    let segments = parse_path_segments(&uri, "/api/v1/webhooks/target/");
    let catalog = require_catalog(&state)?;
    let node = catalog
        .lookup(&segments)
...
fn map_err(e: tiled_catalog::error::CatalogError) -> ServerError {
