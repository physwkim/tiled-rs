//! Translate `crate::core::queries::Query` filters into SQL clauses.
//!
...
impl WhereBuilder {
    ...
    fn push_access_blob_filter(&mut self, filter: &crate::core::queries::AccessBlobFilter) {
        ...
        match q {
            ...
            Query::Regex(r) => {
                ...
                if matches!(builder.dialect, Dialect::Sqlite) {
                    return Err(crate::catalog::error::CatalogError::UnsupportedQuery(
                        format!(
...
impl Catalog {
    ...
    pub async fn get_distinct(
        ...
        specs: bool,
        counts: bool,
    ) -> Result<crate::core::schemas::GetDistinctResponse> {
        let dialect = Dialect::for_pool(self.pool());
        let mut builder = WhereBuilder::new(dialect);
        apply_queries(&mut builder, queries)?;
        let (where_clause, bindings) = builder.finish();
        ...
        let mut resp = crate::core::schemas::GetDistinctResponse::default();
    ...
    async fn run_distinct_group(
        ...
        group_expr: &str,
        counts: bool,
    ) -> Result<Vec<crate::core::schemas::DistinctValueInfo>> {
        ...
        match self.pool() {
            DbPool::Sqlite(pool) => {
                ...
                for row in &q.fetch_all(pool).await? {
                    ...
                    out.push(crate::core::schemas::DistinctValueInfo { value, count });
                }
            }
            DbPool::Postgres(pool) => {
                let mut q = sqlx::query(&sql);
...
fn node_from_sqlite_row(row: &sqlx::sqlite::SqliteRow) -> Result<Node> {
    ...
    let parse_dt = |s: String| -> Result<DateTime<Utc>> {
        DateTime::parse_from_rfc3339(&s)
            ...
            .map_err(|e| {
                crate::catalog::error::CatalogError::Validation(format!("bad timestamp {s}: {e}"))
            })
    };
    Ok(Node {
        id: row.get("id"),
...
mod tests {
    ...
    fn in_empty_list_yields_false() {
        let q = Query::In(crate::core::queries::In {
            key: "color".into(),
            value: vec![],
        });
    ...
    fn notin_empty_list_yields_true() {
        let q = Query::NotIn(crate::core::queries::NotIn {
            key: "color".into(),
            value: vec![],
        });
        let (sql, n) = build(Dialect::Sqlite, &[q]);
    ...
    fn sqlite_regex_is_rejected_postgres_emits_predicate() {
        let q = Query::Regex(crate::core::queries::Regex {
            key: "sample".into(),
            pattern: "Cu.*".into(),
            case_sensitive: true,
        });
        ...
        assert!(matches!(
            err,
            crate::catalog::error::CatalogError::UnsupportedQuery(_)
        ));
    ...
    fn contains_escapes_like_metacharacters() {
        let q = Query::Contains(crate::core::queries::Contains {
            key: "note".into(),
