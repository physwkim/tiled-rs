...
use crate::mongo::run::BlueskyRunAdapter;
...
impl MongoCatalog {
    ...
    async fn runs(&self) -> crate::core::error::Result<Arc<IndexMap<String, AnyAdapter>>> {
        self.runs
...
mod tests {
    ...
    use crate::core::adapters::{AnyAdapter, BaseAdapter, BoxFuture, ContainerAdapter};
    use crate::core::queries::{AccessBlobFilter, KeyLookup, KeysFilter, Like, Query, Regex};
    use crate::core::structures::{ContainerStructure, Spec, StructureFamily};
    use serde_json::json;

    use crate::core::queries::NotIn as NotInQ;
    ...
    fn eq_on_start_field_matches() {
        let a = run(json!({"start": {"plan_name": "scan"}}));
        let q = Query::Eq(crate::core::queries::Eq {
            key: "plan_name".into(),
            value: json!("scan"),
        });
    ...
    fn fulltext_word_match_case_insensitive() {
        ...
        let a = run(json!({"start": {"plan_name": "Scan"}}));
        let q = Query::FullText(crate::core::queries::FullText {
            text: "scan".into(),
        });
        assert!(
            matches_run_query(&a, &q),
    ...
    fn fulltext_does_not_match_partial_word() {
        ...
        let a = run(json!({"start": {"plan_name": "scan"}}));
        let q = Query::FullText(crate::core::queries::FullText { text: "can".into() });
        assert!(
            !matches_run_query(&a, &q),
