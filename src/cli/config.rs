...
use crate::access::{AccessPolicy, PassthroughPolicy, TagBasedPolicy};
use crate::auth::{
    ...
    pub async fn build(&self) -> anyhow::Result<Arc<dyn AccessPolicy>> {
        match self.access_policy.as_str() {
            ...
            "tag_based" => {
                ...
                let auth_db = crate::auth::AuthDb::connect("sqlite::memory:")
                    .await
...
/// One `spec_views[*]` entry. Wire-compatible with `crate::web::SpecViewEntry`
...
impl AuthConfig {
    ...
    pub fn build_internal_authenticators(
        ...
        let mut out: Vec<Arc<dyn crate::auth::Authenticator>> = Vec::new();
        for entry in &self.providers {
            if is_oidc_authenticator(&entry.authenticator) {
...
) -> anyhow::Result<Option<Arc<dyn crate::auth::Authenticator>>> {
    let config: crate::auth::LdapConfig = deserialize_provider_args(&entry.args)
        .with_context(|| format!("authentication.providers '{}': LDAP args", entry.provider))?;
    let auth = crate::auth::LdapAuthenticator::from_config(&entry.provider, config).with_context(
        || {
            format!(
...
fn build_pam_authenticator(
    ...
    let config: crate::auth::PamConfig = deserialize_provider_args(&entry.args)
        .with_context(|| format!("authentication.providers '{}': PAM args", entry.provider))?;
    let auth = crate::auth::PamAuthenticator::from_config(&entry.provider, config);
    Ok(Some(Arc::new(auth)))
}
