...
//! `exchange_code_flow` (given the recovered [`crate::auth::OidcFlowState`])
...
//! `/callback` lives in the auth DB ([`crate::auth::AuthDb::create_oidc_flow_state`]
//! / [`crate::auth::AuthDb::take_oidc_flow_state`]), NOT in this validator. That makes
...
use crate::auth::oidc_flow::OidcFlowState;
...
/// persist (via [`crate::auth::AuthDb::create_oidc_flow_state`], keyed by `state`) so
...
impl ExternalOidcValidator {
    ...
    /// [`crate::auth::AuthDb::create_oidc_flow_state`] so `exchange_code_flow` can
    ...
    /// from [`crate::auth::AuthDb::take_oidc_flow_state`] for this callback's `state`
...
mod tests {
    ...
    // build_authorize_url
    //
    // The pending-state STORE is now DB-backed ([`crate::auth::AuthDb`]); its
