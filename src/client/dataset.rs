...
impl Variable {
    fn ensure_dtype(&self, kind: crate::core::dtype::Kind, itemsize: usize) -> Option<&[u8]> {
        // Tiled servers consistently emit little-endian array buffers.
        match &self.dtype {
            DType::Builtin(b)
                if b.kind == kind
                    && b.itemsize == itemsize
                    && b.endianness == crate::core::dtype::Endianness::Little =>
            {
    ...
    pub fn as_f64_vec(&self) -> Option<Vec<f64>> {
        let bytes = self.ensure_dtype(crate::core::dtype::Kind::Float, 8)?;
        Some(
            bytes
                .chunks_exact(8)
    ...
    pub fn as_f32_vec(&self) -> Option<Vec<f32>> {
        let bytes = self.ensure_dtype(crate::core::dtype::Kind::Float, 4)?;
        Some(
            bytes
                .chunks_exact(4)
    ...
    pub fn as_i64_vec(&self) -> Option<Vec<i64>> {
        let bytes = self.ensure_dtype(crate::core::dtype::Kind::Integer, 8)?;
        Some(
            bytes
                .chunks_exact(8)
    ...
    pub fn as_i32_vec(&self) -> Option<Vec<i32>> {
        let bytes = self.ensure_dtype(crate::core::dtype::Kind::Integer, 4)?;
        Some(
            bytes
                .chunks_exact(4)
    ...
    pub fn as_u64_vec(&self) -> Option<Vec<u64>> {
        let bytes = self.ensure_dtype(crate::core::dtype::Kind::UnsignedInteger, 8)?;
        Some(
            bytes
                .chunks_exact(8)
    ...
    pub fn as_u32_vec(&self) -> Option<Vec<u32>> {
        let bytes = self.ensure_dtype(crate::core::dtype::Kind::UnsignedInteger, 4)?;
        Some(
            bytes
                .chunks_exact(4)
