...
/// of [`crate::adapters::init_storage_npy`]. The server, not the client, decides the
...
pub fn init_storage_zarr(
    ...
    let data_uri = crate::core::file_uri::path_to_file_uri(&store_root).ok_or_else(|| {
...
mod tests {
    ...
    async fn init_storage_creates_store_skeleton_and_resolves_zeros() {
        ...
        assert_eq!(
            data_uri,
            crate::core::file_uri::path_to_file_uri(&store_root).unwrap()
        );
